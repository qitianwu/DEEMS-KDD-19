...
with tf.Session() as sess:
	...
	for _ in range(10000):
		...
		for _, datainput, u_hisinput, u_his_l in DataInput(train_set, u_his_list, train_batch_size, trunc_len, item_count):
			loss_r, loss_reg = model.train(sess, datainput, u_hisinput, u_his_l, learning_rate, keep_prob, lambda_reg)
			iter_num += 1
			loss_r_sum += loss_r
			...
			if model.global_step.eval() % 1000 == 0:
				...
				if AUC > bestAUC:
					model.save(sess, '/cluster/home/it_stu42/KDD/save_model/DREAM.ckpt') 
					bestAUC = AUC
				if P > bestP: 
					bestP = P
				if R > bestR: 
