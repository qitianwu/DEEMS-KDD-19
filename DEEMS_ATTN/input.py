	...
	def __next__(self):
		...
		for t in ts:
			...
			for i in range(5):
				...
				u_his.append(u_his_u)
				u_pos.append(u_pos_u)
				u_his_l.append(len(u_his_u))
				i_his.append(i_his_i)
				i_pos.append(i_pos_i)
				i_his_l.append(len(i_his_i))
		...
		i_posinput = np.zeros([data_len, i_his_maxlength], dtype = np.int32)
		for i, ru in enumerate(i_pos):
			i_posinput[i, :len(ru)] = ru[:len(ru)]

		return self.i, (uid_1, iid_1, label_1), (uid_2, iid_2, label_2), u_hisinput, u_posinput, u_his_l, i_hisinput, i_posinput, i_his_l