...
def _eval(sess, model, test_set_list):
	...
	for i in range(len(test_set_list)):
		...
		u_posinput = np.zeros([len(uid_list), u_his_maxlength], dtype = np.int32)
		for i, ru in enumerate(u_pos):
			u_posinput[i, :len(ru)] = ru[:len(ru)]
		datainput = (uid_list, iid_list, label_list)
		score, loss = model.eval(sess, datainput, u_hisinput, u_posinput, u_his_l)
		score_label_u = []
...
with tf.Session() as sess:
	...
	for _ in range(10000):
		...
		for _, datainput, u_hisinput, u_posinput, u_his_l in DataInput(train_set, u_his_list, train_batch_size, trunc_len, item_count):
			loss_r, loss_reg = model.train(sess, datainput, u_hisinput, u_posinput, u_his_l, learning_rate, keep_prob, lambda_reg)
			iter_num += 1
			loss_r_sum += loss_r
			...
			if model.global_step.eval() % 1000 == 0:
				...
				if AUC > bestAUC:
					model.save(sess, '/cluster/home/it_stu42/KDD/save_model/DIN.ckpt') 
					bestAUC = AUC
				if P > bestP: 
					bestP = P
				if R > bestR: 
		...
		#if model.global_epoch_step.eval() == 3:
